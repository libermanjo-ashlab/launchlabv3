...
router.post("/register", async (req, res, next) => {
  try {
    ...
    const user   = await prisma.user.create({
      data: { email: email.toLowerCase(), password: hashed, name, goal: goal||null, age: age ? parseInt(age) : null },
      select: { id:true, email:true, name:true, goal:true, age:true, createdAt:true },
    });
...
router.put("/me", requireAuth, async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.update({
      ...
      select: { id:true, email:true, name:true, goal:true, age:true },
    });
    res.json({ user });
  } catch(e) { next(e); }
});
