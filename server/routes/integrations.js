...
// ── TWITTER/X OAUTH ───────────────────────────────────────────────────────────
router.get("/twitter/auth", requireAuth, (req, res) => {
  if (!process.env.TWITTER_CLIENT_ID) {
    return res.status(503).json({ error: "Twitter OAuth not configured. Add TWITTER_CLIENT_ID and TWITTER_CLIENT_SECRET to Railway environment variables." });
  }
  const businessId = req.query.businessId || "";
  const state = jwt.sign({ userId: req.userId, businessId }, process.env.JWT_SECRET, { expiresIn: "10m" });
  try {
    const tw = require("../services/twitter");
    const url = tw.buildAuthUrl(state);
    res.json({ url });
...
router.get("/twitter/callback", async (req, res, next) => {
  try {
    ...
    if (error) return res.redirect(`${process.env.CLIENT_URL}?error=twitter_auth_denied`);
    if (!code) return res.redirect(`${process.env.CLIENT_URL}?error=twitter_auth_failed`);

    let userId, businessId;
    try {
      const payload = jwt.verify(state, process.env.JWT_SECRET);
      userId = payload.userId; businessId = payload.businessId;
    } catch {
      return res.redirect(`${process.env.CLIENT_URL}?error=invalid_oauth_state`);
    }
    ...
    const tw = require("../services/twitter");
    const tokens = await tw.exchangeCode(code);
    const profile = await tw.getProfile(tokens.access_token);
    ...
    await prisma.integration.upsert({
      ...
      create: { businessId, provider: "twitter", status: "connected", metadata: JSON.stringify(meta) },
    });
    res.redirect(`${process.env.CLIENT_URL}/hub/${businessId}?tab=settings&twitter=connected`);
  } catch (e) { next(e); }
});
...
router.get("/tiktok/auth", requireAuth, (req, res) => {
  if (!process.env.TIKTOK_CLIENT_KEY) {
    return res.status(503).json({ error: "TikTok OAuth not configured. Add TIKTOK_CLIENT_KEY and TIKTOK_CLIENT_SECRET to Railway environment variables." });
  }
  const businessId = req.query.businessId || "";
  const state = jwt.sign({ userId: req.userId, businessId }, process.env.JWT_SECRET, { expiresIn: "10m" });
  try {
    const tt = require("../services/tiktok");
    const url = tt.buildAuthUrl(state);
    res.json({ url });
  } catch (e) { res.status(503).json({ error: e.message }); }
});
...
router.get("/tiktok/callback", async (req, res, next) => {
  try {
    const { code, state, error } = req.query;
    if (error) return res.redirect(`${process.env.CLIENT_URL}?error=tiktok_auth_denied`);
    if (!code) return res.redirect(`${process.env.CLIENT_URL}?error=tiktok_auth_failed`);
    ...
    const tt = require("../services/tiktok");
    const tokens = await tt.exchangeCode(code);
    const profile = await tt.getProfile(tokens.access_token);
    ...
    await prisma.integration.upsert({
      where:  { businessId_provider: { businessId, provider: "tiktok" } },
      update: { status: "connected", metadata: JSON.stringify(meta) },
      create: { businessId, provider: "tiktok", status: "connected", metadata: JSON.stringify(meta) },
    });
    res.redirect(`${process.env.CLIENT_URL}/hub/${businessId}?tab=settings&tiktok=connected`);
  } catch (e) { next(e); }
});
